...
class AreaCircleDialog extends JInternalFrame {
   ...
   private void areaActionPerformed() {
      
      int input = 0;
      try {
         input = Integer.parseInt(tf.getText());
         double val = Math.PI *Math.pow(input,2);
         int area = (int) val;
         lbl.setText("Area of circle with the radius " +input+ " is " +area);
      }
      catch(Exception e) {
   ...
   private AreaCircleDialog() {
      ...
      super("Area", false, true, false, false);
      
      tf = new JTextField(10);
      btn = new JButton("Area of a Circle?");
